import axios from "axios";
  ...
  export async function fetchMovieSource(tmdbId: number): Promise<MovieDownloadInfo> {
    ...
    const match =
      results.find(
        (r: any) =>
          r.type === "Movie" && r.title?.toLowerCase() === title.toLowerCase()
      ) ??
      results.find(
        (r: any) =>
          r.type === "Movie" &&
    ...
    if (!match?.id) {
      throw new Error(
        `No stream source found for "${title}" — try a different TMDB ID`
      );
    }
    ...
    const sorted = [...sources].sort((a: any, b: any) => {
      ...
      const qa = QUALITY_ORDER.indexOf(
        (a.quality ?? "auto").toString().toLowerCase()
      );
      const qb = QUALITY_ORDER.indexOf(
        (b.quality ?? "auto").toString().toLowerCase()
      );
